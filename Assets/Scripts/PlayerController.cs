...
public class PlayerController : MonoBehaviour {
    ...
    public GameObject disappearAnim;
    ...
    private void OnCollisionEnter2D(Collision2D other) {
        if (other.gameObject.CompareTag("KillBox")) {
            ...
            Instantiate(disappearAnim, transform.position, transform.rotation);

            if(GameManager.Instance.gameMusicSource.isPlaying){
                GameManager.Instance.gameMusicSource.Stop();
            }

            GameManager.Instance.loseMusicSource.Play();
